...
class TrendingViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, MFMailComposeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let titleNib = UINib(nibName: "Title1TableViewCell", bundle: nil)
        tableView.register(titleNib, forCellReuseIdentifier: "title1")
    ...
    @objc func refresh(sender: UIRefreshControl) {
        ...
        //24時間以内に投稿された上位30位の投稿のみ表示
        let postsRef = Firestore.firestore().collection("posts").whereField("date", isGreaterThanOrEqualTo: modifiedDate).limit(to: 30)
        listener = postsRef.addSnapshotListener() { (querySnapshot, error) in
            ...
            self.postArray.sort {
                $0.allComments.count > $1.allComments.count
            }
            self.postArray.sort {
                $0.likes.count > $1.likes.count
            }
            self.postArray.sort {
                $0.viewer.count > $1.viewer.count
            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            // configure separator
            do {
                let hasSeparator = indexPath.section == 0
                cell.configure(hasSeparator: hasSeparator)
            }
            return cell
        }else {
